...
namespace Glutzic\GithubWrikeBundle\Client;
...
class WrikeClient
{
    ...
    public function setAccessInfo(\Glutzic\GithubWrikeBundle\Entity\OAuthTokenInfo $info)
    {
        $this->access_token = $info->getToken();
        $this->access_token_secret = $info->getSecret();
    }
    ...
    public function getAccessToken($token = null, $secret = null)
    {
        ...
        $secret = $secret ? : $this->access_token_secret;
     ...
     * 
     */
    private function addFolder($title = false, $description = false, $sharedUsers = false, $parents = false, $account = false,
        $asyncKey = false)
    {
        ...
        if ($sharedUsers)
            $params['sharedUsers'] = $sharedUsers;
        if ($parents)
     ...
     * 
     */
    private function updateFolder($id = false, $title = false, $description = false, $sharedUsers = false, $parents = false,
        $asyncKey = false)
    {
     ...
     * 
     */
    public function addTask($title = false, $description = false, $status = false, $importance = false, $startDate = false,
        $dueDate = false, $duration = false, $responsibleUsers = false, $sharedUsers = false, $parents = false, $account = false,
        $asyncKey = false)
    {
        $url = "https://www.wrike.com/api/json/v2/wrike.task.add";
        $params = array();

        if ($responsibleUsers && !empty($this->github_user_ids[$responsibleUsers]))
            $responsibleUsers = $this->github_user_ids[$responsibleUsers];
        ...
        if ($responsibleUsers)
            $params['responsibleUsers'] = $responsibleUsers;
        if ($sharedUsers)
            $params['sharedUsers'] = $sharedUsers;
        if ($parents)
            $params['parents'] = $parents;
     ...
     * 
     */
    private function updateTask($id = false, $title = false, $description = false, $status = false, $importance = false,
        $startDate = false, $dueDate = false, $duration = false, $responsibleUsers = false, $sharedUsers = false,
        $parents = false, $asyncKey = false)
    {
    ...
    public function github_sync($json)
    {
        ...
        if (!empty($github->action)) {
            ...
            if (!empty($github->issue) && empty($github->issue->pull_request->html_url)) {
                ...
                if (empty($folder_id)) {
                    $wrikeResponses['folderResponse'] = $this->addFolder($issue->milestone_title,
                        $issue->milestone_id.'::'.$issue->milestone_num, $shared_users);
                ...
                // sync task in wrike
                if (empty($task_id)) {
                    $wrikeResponses['taskResponse'] = $this->addTask($issue->title, $issue->description, $issue->status, '1',
                        false, false, false, $issue->assignee, $shared_users, $parent_folder);
                    ...
                    if (empty($wrike_task->startDate))
                        $wrike_task->startDate = false;
                    if (empty($wrike_task->dueDate))
                        $wrike_task->dueDate = false;
                    if (empty($wrike_task->duration))
                        $wrike_task->duration = false;
                    if (empty($wrike_task->importance))
                        $wrike_task->importance = '1';
                    if (empty($wrike_task->parents))
                        $wrike_task->parents = false;

                    $wrike_task->responsibleUsers = (array) $wrike_task->responsibleUsers;
                    $wrike_task->responsibleUsers = ( ( empty($wrike_task->responsibleUsers) ) ? $issue->assignee : implode(',',
                                $wrike_task->responsibleUsers) );

                    $wrikeResponses['taskResponse'] = $this->updateTask($task_id, $issue->title, $issue->description,
                        $issue->status, $wrike_task->importance, $wrike_task->startDate, $wrike_task->dueDate,
                        $wrike_task->duration, $wrike_task->responsibleUsers, $shared_users, $parent_folder);
                    $taskDecode = json_decode($wrikeResponses['taskResponse']);
